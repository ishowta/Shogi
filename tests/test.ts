...
import {
	...
	MoveNotAllowedError,
	Ok,
	PlacementNotAllowedError,
	Shogi,
	Checkmate,
	Foul
} from "../src/library/shogi/shogi"
...
it("相手の駒は動かせない", () => {
	const res: Ok | MoveNotAllowedError | Foul | Checkmate = shogi.move(c(9, 1), c(9, 2))
...
it("既に成っているのに成ろうとする", () => {
	...
	const res: Ok | MoveNotAllowedError | Foul | Checkmate = shogi.move(c(2, 2), c(1, 1), true) // 11角成(🙅),香取り
	assert(res.type === "move_error" && res.reason instanceof CantPromoteError)
})

it("成ることのできない駒なのに成ろうとする", () => {
	const res: Ok | MoveNotAllowedError | Foul | Checkmate = shogi.move(c(5, 9), c(5, 8), true) // 58王成(🙅)
	assert(res.type === "move_error" && res.reason instanceof CantPromoteError)
})
...
it("打ち歩詰め", () => {
	...
	const res: Ok | PlacementNotAllowedError | Foul | Checkmate = shogi.placeHandPiece(hu, c(5, 2)) // 52歩打,王手(🙅)
...
it("千日手", () => {
	...
	const thousandDaysHand = shogi.move(c(6, 9), c(6, 8))
	assert(thousandDaysHand.type === "foul" && thousandDaysHand.reason instanceof ThousandDaysFoul) // 68金
})
...
it("王手放置", () => {
	...
	const res: Ok | MoveNotAllowedError | Foul | Checkmate = shogi.move(c(9, 3), c(9, 4)) // 94歩(🙅)
